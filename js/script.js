        ...
        (function () {
            ...
            function showToast(msg, type = 'success', actionText = null, actionFn = null) {
                ...
                t.innerHTML = `<i class="fas ${icons[type] || icons.info} toast-icon"></i><span class="toast-msg">${msg}</span>${actionText ? `<button class="toast-action" style="background:var(--primary-soft);color:var(--primary);border:none;padding:0.25rem 0.7rem;border-radius:30px;font-weight:600;font-size:0.75rem;cursor:pointer;">${actionText}</button>` : ''}`;
                if (actionText && actionFn) {
            ...
            function updateCartUI() {
                ...
                <div class="cart-item">
                    ...
                    <div class="cart-item-info">
                        <h4>${i.name}</h4>
                        <div class="price" style="color:var(--primary);font-weight:600;">$${i.price.toFixed(2)}</div>
                        <div class="cart-item-actions">
                    ...
                    <button class="cart-item-remove" data-id="${i.id}" style="background:none;border:none;color:var(--text-muted);cursor:pointer;"><i class="fas fa-times"></i></button>
                </div>
            `).join('');
                itemsEl.querySelectorAll('.qty-dec').forEach(b => b.addEventListener('click', function () { updateQty(parseInt(this.dataset.id), -1); }));
                itemsEl.querySelectorAll('.qty-inc').forEach(b => b.addEventListener('click', function () { updateQty(parseInt(this.dataset.id), 1); }));
            ...
            function updateCompareUI() {
                ...
                if (compareList.length > 0) {
                    ...
                    items.innerHTML = compareList.map(id => {
                        const p = findProductById(id);
                        if (!p) return '';
                        return `<div class="compare-item"><img src="${p.image}" alt="${p.name}"><span>${p.name.length > 18 ? p.name.slice(0, 18) + '...' : p.name}</span><button class="remove-compare" data-id="${id}"><i class="fas fa-times"></i></button></div>`;
                    }).join('');
            ...
            // ============================================================
            // CATEGORY PAGE — FIXED: separate from main
            // ============================================================
            function showCategoryPage(cat) {
                const products = CATEGORY_PRODUCTS[cat];
                ...
                // Hide main sections, footer, newsletter
                document.getElementById('mainSections').style.display = 'none';
                document.querySelector('.newsletter-section').style.display = 'none';
                document.querySelector('footer').style.display = 'none';
                document.getElementById('dashboardWrapper').classList.remove('active');
            ...
            function renderDashboardPage(page) {
                ...
                if (page === 'overview') {
                    ...
                    <div class="content-grid">
                        ...
                        <div class="loyalty-card">
                            <div class="tier-badge">${tier.icon} ${tier.name} Member</div>
                            <div class="points-big">${points.toLocaleString()}</div>
                            <div style="opacity:0.8;font-size:0.85rem;margin-bottom:1rem;">Reward Points</div>
                            <div class="tier-progress"><div class="tier-progress-bar" style="width:${progress}%"></div></div>
                        </div>
                        <div class="content-card" style="grid-column:1/-1;">
                            <div class="card-header"><h3><i class="fas fa-history"></i> Recent Orders</h3></div>
                            ${(userData.orders || []).length === 0 ? '<p style="color:var(--text-muted);text-align:center;padding:1rem;">No orders yet</p>' : userData.orders.slice(0, 3).map(o => `<div style="display:flex;justify-content:space-between;padding:0.7rem 0;border-bottom:1px solid var(--border);"><div><strong>#${o.id}</strong><div style="font-size:0.75rem;color:var(--text-muted);">${new Date(o.date).toLocaleDateString()}</div></div><div style="font-weight:700;color:var(--primary);">$${o.total.toFixed(2)}</div></div>`).join('')}
                        </div>
                    </div>
                `;
                } else if (page === 'orders') {
                ...
                `;
                    ...
                    <div class="content-card">
                        ...
                        ${(userData.addresses || []).length === 0 ? '<p style="text-align:center;color:var(--text-muted);padding:1rem;">No addresses saved</p>' : userData.addresses.map(a => `<div style="padding:0.8rem;background:var(--surface-2);border-radius:10px;margin-bottom:0.5rem;"><strong>${a.label || 'Address'}</strong><br>${a.street}, ${a.city}, ${a.state} ${a.zip}</div>`).join('')}
                    </div>
                `;
                } else if (page === 'loyalty') {
                    const points = getUserPoints();
                    ...
                    <div class="content-card">
                        <div class="loyalty-card" style="margin-bottom:1.5rem;">
                            ...
                            <div class="points-big">${points.toLocaleString()}</div>
                            <div style="opacity:0.8;margin-bottom:1rem;">Points</div>
                            <div class="tier-progress"><div class="tier-progress-bar" style="width:${progress}%"></div></div>
                        </div>
                        <h3 style="margin-bottom:1rem;">How to Earn</h3>
                        <p style="padding:0.5rem 0;border-bottom:1px solid var(--border);"><i class="fas fa-shopping-bag" style="color:var(--primary);margin-right:0.5rem;"></i> 10 points per $1 spent</p>
                        <p style="padding:0.5rem 0;border-bottom:1px solid var(--border);"><i class="fas fa-star" style="color:var(--primary);margin-right:0.5rem;"></i> 50 points per review</p>
                        <p style="padding:0.5rem 0;"><i class="fas fa-user-plus" style="color:var(--primary);margin-right:0.5rem;"></i> 500 points per referral</p>
                    </div>
                `;
                } else if (page === 'profile') {
                    content.innerHTML = `
            ...
            function trackOrder(orderId) {
                ...
                <div class="checkout-panel active">
                    <h3><i class="fas fa-truck"></i> Order Tracking</h3>
                    <div style="background:var(--surface-2);padding:1rem;border-radius:10px;margin-bottom:1rem;">
                    ...
                    <button class="btn" style="width:100%;justify-content:center;" onclick="closeCheckout()">Close</button>
                </div>
            `;
                document.getElementById('checkoutModal').classList.add('active');
            }
            ...
            function renderCheckout() {
                ...
                if (checkoutStep === 1) {
                    ...
                    <div class="checkout-panel active">
                        ...
                        <div class="coupon-section">
                            <label style="font-weight:600;font-size:0.85rem;margin-bottom:0.5rem;display:block;">Coupon Code</label>
                            <div class="coupon-input-row">
                ...
                `;
                    ...
                    <div class="checkout-panel active">
                        ...
                        <div class="payment-methods">
                            <div class="payment-method active" data-method="card"><i class="fas fa-credit-card"></i><div><div style="font-weight:600;font-size:0.9rem;">Credit / Debit Card</div><div style="font-size:0.8rem;color:var(--text-muted);">Visa, Mastercard</div></div></div>
                            <div class="payment-method" data-method="paypal"><i class="fab fa-paypal"></i><div><div style="font-weight:600;font-size:0.9rem;">PayPal</div><div style="font-size:0.8rem;color:var(--text-muted);">Pay with PayPal</div></div></div>
                            <div class="payment-method" data-method="cod"><i class="fas fa-money-bill-wave"></i><div><div style="font-weight:600;font-size:0.9rem;">Cash on Delivery</div><div style="font-size:0.8rem;color:var(--text-muted);">Pay when delivered</div></div></div>
                        </div>
                        <div id="cardFields">
                            <div class="form-group"><label>Card Number</label><input type="text" id="cardNum" placeholder="1234 5678 9012 3456"></div>
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                ...
                `;
                    ...
                    <div class="checkout-panel active">
                        ...
                        <div class="checkout-summary">
                            <div style="margin-bottom:1rem;"><strong>Ship to:</strong><br>${s.name}<br>${s.street}<br>${s.city}, ${s.state} ${s.zip}</div>
                            <div style="margin-bottom:1rem;"><strong>Payment:</strong> ${p.method === 'card' ? 'Card' : p.method === 'paypal' ? 'PayPal' : 'COD'}</div>
            ...
            function placeOrder() {
                ...
                <div class="checkout-panel active">
                    <div class="order-success">
                        ...
                        <p style="color:var(--success);font-weight:600;margin-bottom:1.5rem;"><i class="fas fa-coins"></i> You earned ${earned} points!</p>
                        <button class="btn" style="width:100%;justify-content:center;" onclick="closeCheckout()">Continue Shopping</button>
                    </div>
                </div>
            `;
                cart = [];
            ...
            function renderAdminPanel(panel) {
                ...
                `;
                    ...
                    body.innerHTML = `
                    <div class="admin-panel active">
                        <h3 style="margin-bottom:1rem;">All Products (${allProducts.length})</h3>
                        <div style="max-height:400px;overflow-y:auto;">
                            <table class="admin-table">
                                <thead><tr><th>Image</th><th>Name</th><th>Price</th><th>Stock</th><th>Category</th></tr></thead>
                                <tbody>${allProducts.slice(0, 20).map(p => `<tr><td><img src="${p.image}" style="width:40px;height:40px;object-fit:cover;border-radius:6px;"></td><td>${p.name}</td><td>$${p.price.toFixed(2)}</td><td>${p.stock}</td><td>${p.category}</td></tr>`).join('')}</tbody>
                            </table>
                        </div>
                    </div>
                `;
            ...
            // ============================================================
            function initPWA() {
                // Show banner after 8 seconds only once
            ...
            function handleSearch(q) {
                ...
                if (matches.length === 0) {
                    box.innerHTML = `<div style="padding:1.5rem;text-align:center;color:var(--text-muted);">No results for "${q}"</div>`;
                } else {
            ...
            function init() {
                ...
                // Chat badge - hide if already seen
                ...
                document.getElementById('searchInput').addEventListener('input', function () { handleSearch(this.value); });
                document.getElementById('searchInput').addEventListener('focus', function () { if (this.value.trim()) handleSearch(this.value); });
                document.addEventListener('click', e => { if (!e.target.closest('.search-wrapper')) document.getElementById('searchSuggestions').classList.remove('active'); });
