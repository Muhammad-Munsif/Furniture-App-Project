        ...
        (function() {
            ...
            // ============================================================
            const FEATURED = [
                { id: 1, name: 'Luxury Velvet Sofa', price: 899.99, image: 'https://images.unsplash.com/photo-1567538096630-e0c55bd6374c?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80', desc: 'Elegant velvet with premium cushioning.', isNew: true },
                { id: 2, name: 'Modern Armchair', price: 349.99, image: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80', desc: 'Ergonomic design with stylish fabric.', isNew: false },
                { id: 3, name: 'Oak Dining Table', price: 599.99, image: 'https://images.unsplash.com/photo-1592078615290-033ee584e267?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80', desc: 'Solid oak, extendable for gatherings.', isNew: true },
                { id: 4, name: 'Minimalist Bed Frame', price: 749.99, image: 'https://images.unsplash.com/photo-1540574163026-643ea20ade25?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80', desc: 'Clean lines, sturdy construction.', isNew: false }
            ];
            ...
            const quickViewName = $('#quickViewName');
            const quickViewPrice = $('#quickViewPrice');
            const quickViewDesc = $('#quickViewDesc');
            const quickViewQtyEl = $('#quickViewQty');
            const quickViewQtyDec = $('#quickViewQtyDec');
            const quickViewQtyInc = $('#quickViewQtyInc');
            const quickViewAddToCart = $('#quickViewAddToCart');
            const floatingCartBtn = $('#floatingCartBtn');
            const seeAllProducts = $('#seeAllProducts');
            ...
            // 5. PRODUCT RENDERING
            // ============================================================
            function renderProductGrid(products, container, addToCartHandler, showSkeleton = false) {
                if (showSkeleton) {
                ...
                container.innerHTML = products.map(p => {
                    ...
                    <div class="product-card" data-id="${p.id}">
                        ${isNew ? '<span class="sale-badge">New</span>' : ''}
                        <button class="wishlist-btn ${inWishlist ? 'active' : ''}" data-id="${p.id}">
                            <i class="fas fa-heart"></i>
                        </button>
                        <img src="${p.image}" alt="${p.name}" loading="lazy" onclick="window.handleQuickView(${p.id})">
                        <div class="product-body">
                            ...
                            <div class="product-actions">
                                ...
                                <button class="btn btn-small btn-outline quick-view-btn" data-id="${p.id}">
            ...
            function renderRecentlyViewed() {
                ...
                renderProductGrid(products, recentlyViewedGrid, addToCart);
            }
            ...
            quickViewQtyDec.addEventListener('click', function() {
                if (quickViewQty > 1) {
                    quickViewQty--;
            ...
            quickViewQtyInc.addEventListener('click', function() {
                quickViewQty++;
                quickViewQtyEl.textContent = quickViewQty;
            });
            ...
            quickViewAddToCart.addEventListener('click', function() {
                ...
                const existing = cart.find(item => item.id === quickViewProduct.id);
                if (existing) {
                    existing.qty += quickViewQty;
                } else {
                    cart.push({
                ...
                addToRecentlyViewed(quickViewProduct.id);
                closeQuickView();
            });
            ...
            // Expose quick view to global scope
            ...
            function showCategoryPage(categoryKey) {
                ...
                categoryBreadcrumb.textContent = displayName;
            ...
            // ============================================================
            // 15. MEGA MENU CATEGORY LINKS
            // ============================================================
            document.querySelectorAll('.mega-menu-col a[data-category]').forEach(link => {
            ...
            function init() {
                ...
                renderProductGrid(FEATURED, productGrid, addToCart, true);

                // Then render actual products after delay
                setTimeout(() => {
                ...
                // Wishlist open
                $('#wishlistOpen').addEventListener('click', function() {
                    if (wishlist.length === 0) {
                        showToast('Your wishlist is empty', 'fa-heart');
                        return;
                    }
                    showToast(`You have ${wishlist.length} items in wishlist`, 'fa-heart');
                });
