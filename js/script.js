...
(function () {
    ...
    // ============ DATA (abbreviated - products same as before) ============
    const FEATURED = [
    ...
    function animateCounter(el, target, duration = 1200, isPrice = false) {
        ...
        function update(currentTime) {
            ...
            if (isPrice) el.textContent = '$' + current.toFixed(0);
            else el.textContent = Math.round(current).toLocaleString();
            if (progress < 1) requestAnimationFrame(update);
            else { el.textContent = isPrice ? '$' + targetNum.toFixed(0) : targetNum.toLocaleString(); }
        }
    ...
    function initScrollReveal() {
        const reveals = document.querySelectorAll('.reveal, .reveal-left, .reveal-right, .reveal-zoom, .reveal-rotate');
    ...
    function renderDashboardPage(page) {
        ...
        if (page === 'overview') {
            ...
            content.innerHTML = `
                    <div class="content-grid" style="margin-bottom:1.2rem;">
                        <div class="content-card">
                            <div class="card-header">
                                <h3><i class="fas fa-chart-line"></i> Spending Overview</h3>
                                <span class="card-action">Last 7 days</span>
                            </div>
                            <div class="mini-chart">
                                ${chartData.map((v, i) => `<div class="bar" style="height:${v}%; animation-delay:${i * 0.08}s;" data-value="$${v}"></div>`).join('')}
                            </div>
                            <div style="display:flex;justify-content:space-between;font-size:0.68rem;color:var(--text-muted);margin-top:0.5rem;">
                            ...
                            <div class="loyalty-content">
                                ...
                                <div class="loyalty-progress-wrap">
                                    <div class="loyalty-progress-label">
                                        <span>${tier.name}</span>
                                        <span>${tier.next ? (tier.next - points).toLocaleString() + ' pts to go' : 'Max tier!'}</span>
                                    </div>
                    ...
                    <div class="content-card" style="grid-column:1/-1;">
                        <div class="card-header">
                            ...
                            <span class="card-action" onclick="switchDashboardPage('orders')">View All <i class="fas fa-arrow-right"></i></span>
                        </div>
                ...
                return `
                                    <div class="timeline-order">
                                        <div class="order-status-dot ${statusClass}">
            ...
            content.innerHTML = `
                    <div class="dash-page-header">
                        <div>
                            <h2><i class="fas fa-map-pin"></i> My Addresses</h2>
            ...
            content.innerHTML = `
                    <div class="dash-page-header">
                        <div>
                            <h2><i class="fas fa-coins"></i> Rewards</h2>
                        ...
                        <div class="loyalty-card-modern" style="grid-column:1/-1;">
                            <div class="loyalty-content">
                                ...
                                <div class="loyalty-progress-wrap">
                                    <div class="loyalty-progress-label">
                                        <span>${tier.name}</span>
                                        <span>${tier.next ? (tier.next - points).toLocaleString() + ' pts to next tier' : 'Maximum tier reached!'}</span>
                                    </div>
                                    <div class="loyalty-progress">
                                        <div class="loyalty-progress-bar" style="width:${progress}%"></div>
                                    </div>
