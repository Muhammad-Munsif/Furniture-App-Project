...
(function () {
    ...
    // User schema: { id, name, email, password, status, orders, totalSpent, joined, cart }
    function getUsers() {
        return JSON.parse(localStorage.getItem('furni_users_db')) || [];
    }

    function saveUsers(users) {
        localStorage.setItem('furni_users_db', JSON.stringify(users));
    }
    ...
    function updateUserInDB(id, updates) {
        const users = getUsers();
    ...
    // Add order to user
    function addOrderToUser(userId, orderTotal) {
        const users = getUsers();
    ...
    const savedUser = JSON.parse(localStorage.getItem('furni_current_user')) || null;
    if (savedUser) {
        const users = getUsers();
        ...
        if (found) {
            ...
            localStorage.removeItem('furni_current_user');
        }
    }
    ...
    const totalUsersSpan = $('#totalUsers');
    const activeUsersSpan = $('#activeUsers');
    const totalOrdersSpan = $('#totalOrders');
    const totalRevenueSpan = $('#totalRevenue');
        ...
        if (users.length === 0) {
            userTableBody.innerHTML = `
                        <tr>
                            <td colspan="8" style="text-align:center; padding: 2rem; color: var(--text-muted);">
                                ...
                                No users registered yet. Add sample users or sign up!
                            </td>
                        </tr>
                    `;
        } else {
            userTableBody.innerHTML = users.map(user => `
                        <tr>
                            ...
                            <td><a href="mailto:${user.email}" style="color: var(--primary);">${user.email}</a></td>
            ...
            userTableBody.querySelectorAll('.delete-btn').forEach(btn => {
                btn.addEventListener('click', function () {
                    ...
                    if (confirm(`Delete user #${id}? This action cannot be undone.`)) {
                        ...
                        renderUserTable();
                        updateTableStats();
                        showToast(`User #${id} deleted`, 'fa-trash');
            ...
            // Attach edit events (simple prompt for name/status)
            userTableBody.querySelectorAll('.edit-btn').forEach(btn => {
                btn.addEventListener('click', function () {
                    ...
                    if (newName && newName.trim()) {
                        const newStatus = confirm('Set status to active? Click OK for active, Cancel for inactive');
                        updateUserInDB(id, {
                            name: newName.trim(),
                        ...
                        renderUserTable();
                        updateTableStats();
                        showToast(`User #${id} updated`, 'fa-edit');
                        // Update current user if it's them
                        if (currentUser && currentUser.id === id) {
                            const updated = getUsers().find(u => u.id === id);
                            if (updated) {
                                currentUser = updated;
                                localStorage.setItem('furni_current_user', JSON.stringify(currentUser));
                                updateUserUI();
                            }
                        }
                    }
    ...
    function addSampleUsers() {
        ...
        const sampleEmails = ['alice@example.com', 'bob@example.com', 'carol@example.com', 'david@example.com',
            'emma@example.com'
        ];
        const users = getUsers();
        let count = 0;
        sampleNames.forEach((name, i) => {
            ...
            if (!users.find(u => u.email === email)) {
                const user = addUserToDB(name, email, 'password123');
                // Add some orders to sample users
                if (i % 2 === 0) {
                    const orderTotal = Math.round((Math.random() * 500 + 100) * 100) / 100;
                    addOrderToUser(user.id, orderTotal);
                }
                if (i % 3 === 0) {
                    updateUserInDB(user.id, { status: 'inactive' });
                }
                count++;
            }
        });
        renderUserTable();
        updateTableStats();
        showToast(`${count} sample users added!`, 'fa-users');
    }
    ...
    function saveCart() {
        ...
        localStorage.setItem('furni_cart_backup', JSON.stringify(cart));
    }
    ...
    function init() {
        ...
        // Load cart from current user or backup
        if (currentUser && currentUser.cart) {
            cart = currentUser.cart;
        } else {
            const backup = JSON.parse(localStorage.getItem('furni_cart_backup')) || [];
            cart = backup;
        }
        ...
        $('#signupForm').addEventListener('submit', function (e) {
            ...
            const newUser = addUserToDB(name, email, password);
            currentUser = newUser;
            cart = [];
            localStorage.setItem('furni_current_user', JSON.stringify(currentUser));
            updateUserUI();
        ...
        $('#checkoutBtn').addEventListener('click', function () {
            ...
            const total = cart.reduce((sum, i) => sum + i.price * i.qty, 0);
            // Add order to user
            addOrderToUser(currentUser.id, total);
        ...
        // Admin table actions
        $('#addSampleUsers').addEventListener('click', addSampleUsers);
        $('#refreshTable').addEventListener('click', function () {
            renderUserTable();
            updateTableStats();
            showToast('Table refreshed', 'fa-sync');
        });
        $('#clearAllUsers').addEventListener('click', function () {
            if (confirm('⚠️ Delete ALL users? This action cannot be undone!')) {
                if (confirm('Are you sure? All user data will be permanently deleted.')) {
                    saveUsers([]);
                    if (currentUser) {
                        currentUser = null;
                        localStorage.removeItem('furni_current_user');
                    ...
                    renderUserTable();
                    updateTableStats();
                    showToast('All users cleared', 'fa-trash');
                }
            }
        });
